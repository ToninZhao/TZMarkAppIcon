...
function generateIcon() {
    ...
    height_0=`expr ${height} / 3`
    height_1=$((${height} - ${height_0}))
    convert blur-original.png -crop ${width}x${height_1}+0+${height_0} crop-blur-original.png

    # 加字
    point_size=$(((8 * $height) / 58))

    convert -background none -fill gray -pointsize ${point_size} -gravity center caption:"${caption}" crop-blur-original.png +swap -composite label.png
